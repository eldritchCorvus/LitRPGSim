...
class PlayState extends MusicBeatState
{
	...
	var bgBoppers:SpaceBoppers;
	//DIS IS WHERE DA BOPPERS AT
	//This defines the BOPPER function in this file. Refer to SpaceBoppers.hx
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			/*
			  FUTURE ME, LISTEN UP:
			  Right now, some stages are separated by song. I left it like this in case the background DRASTICALLY changes
			  
			  HOWEVER, if all that's gonna be added are the boppers, MAKE A NEW .HX (like in bg girls), and TELL IT TO CHANGE THERE
			  if you're also going to try making a bg asset randomizer, ALSO MAKE A NEW .HX AND THEN JUST CALL BACK TO IT HERE
			  
			  having to change EVERY CASE every time something changes is gonna give you a headache
			  save yourself the redundant code
			  
			  my final message change da worl good bye
			*/
			case 'spark' | 'thunderstruck':
			{
					curStage = 'spaceStage';
					//THIS BACKGROUND shouldn't have a lot of motion, add some peeps in the bg and you're done
					//once you finish the FUNNY SPACE RANDOMIZER .hx, come back here and add the line
					
					defaultCamZoom = 0.80;
					
					var bgSpace:FlxSprite = new FlxSprite(-300, -300).loadGraphic(Paths.image('watt/space'));
					bgSpace.active = false;
					bgSpace.antialiasing = true;
					bgSpace.scrollFactor.set(0.12, 0.12);
					bgSpace.setGraphicSize(Std.int(bgSpace.width * 0.6));
					bgSpace.updateHitbox();
					add(bgSpace);
					//SPACE BACKGROUND, come back here to set up the randomizer funny function
					
					var bgHull:FlxSprite = new FlxSprite(-550, -300).loadGraphic(Paths.image('watt/hull'));
					bgHull.active = false;
					bgHull.antialiasing = true;
					bgHull.scrollFactor.set(0.9, 0.9);
					bgHull.setGraphicSize(Std.int(bgHull.width * 0.6));
					bgHull.updateHitbox();
					add(bgHull);
					//hull, windows go after this
					
					bgBoppers = new SpaceBoppers( -680, -220);
					bgBoppers.scrollFactor.set(0.9, 0.9);
					bgBoppers.setGraphicSize(Std.int(bgBoppers.width * 0.8));
					
					if (SONG.song.toLowerCase() == 'thunderstruck')
						{
							bgBoppers.getBusy();
					}
					...
					var bgWindows:FlxSprite = new FlxSprite(-450, -50).loadGraphic(Paths.image('watt/windows'));
					
					bgWindows.active = false;
					bgWindows.antialiasing = true;
					bgWindows.scrollFactor.set(0.9, 0.9);
					bgWindows.setGraphicSize(Std.int(bgHull.width * 0.8));
					bgWindows.updateHitbox();
					add(bgWindows);
					//these r ...da window
					//they're transparent, so load up the boppers behind this one
					
					var bgCyber:FlxSprite = new FlxSprite(-1200, 600).loadGraphic(Paths.image('watt/cgroundbg'));
					bgCyber.active = false;
					bgCyber.antialiasing = true;
					bgCyber.scrollFactor.set(0.9, 0.9);
					bgCyber.setGraphicSize(Std.int(bgCyber.width * 0.6));
					bgCyber.updateHitbox();
					add(bgCyber);
					//bg ground
					
					var fgCyber:FlxSprite = new FlxSprite( -2150, 400).loadGraphic(Paths.image('watt/cyberfground'));
					// I WILL NEVER MAKE AN IMAGE LARGER THAN 1200 PX. I WILL NEVER MAKE AN IMAGE LARGER THAN 1200PX. I WILL NEVER
					fgCyber.active = false;
					fgCyber.antialiasing = true;
					add(fgCyber);
			}
			case 'deathzap':
			{
			...
			case 'wattathon':
			{
					...
					if (SONG.song.toLowerCase() == 'wattathon')
						{
							bgBoppers.getBusy();
					}
			...
			case 'cyberstorm':
			{
					curStage = 'spaceStageTakeover';
					//bolt is here now, whoops
					...
					if (SONG.song.toLowerCase() == 'cyberstorm')
						{
							bgBoppers.getBusy();
					}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.x += 180;
				gf.y += 300;
			case 'spaceStage' | 'spaceStageRave'| 'spaceStageTakeover':
				boyfriend.x += 200;
				boyfriend.y += 30;
		}
	...
	*/
	...
	override function beatHit()
	{
		...
		//lol, thanks cameron, i'm taking this
		if (curSong.toLowerCase() == 'cyberstorm' && curBeat >= 150 && curBeat < 255 && camZooming && FlxG.camera.zoom < 1.35)
		{
